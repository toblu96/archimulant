...
│   ├── logger.ts                 # Logger port (wide-event context accumulator).
...
├── middleware/
│   └── 00.context.ts             # Per-request scope. Runs on every request.
...
### Logging — evlog wide events behind a port, two lifetimes
...
evlog's Nitro integration is event-bound — the request logger is only reachable via `useLogger(event)`. But singletons (DB pools, message bus connections, the simulation engine) fire events outside any request and need their own logger. So we keep **two `Logger` instances** with different lifetimes:

- **`appLogger`** — singleton, lives on `AppContainer`, created at boot via evlog standalone (`createLogger()` from `evlog`). Used by anything outside a request: composition root, plugin hooks, shutdown handlers, pool event listeners, scheduled jobs.
- **`logger`** — request-scoped, lives on `event.context.scope`, created per request from `useLogger(event)`. Used by use cases and per-request adapter calls.

Both implement the same `Logger` port. Different instances, different lifetimes.
...
export const createPostgresPool = (config: {
  ...
  return {
    ...
    async shutdown() {
      config.appLogger.set({ event: 'db.pool.shutdown' })
      await pool.end()
    },
  }
}
...
- **Wide events, not log levels.** Use `set(...)` to accumulate context. Don't add `info`/`warn`/`error` methods to the port; severity is determined by the response status and any attached error.
- **Two lifetimes, same port.** `appLogger` for singleton/lifecycle, `logger` for request-scoped. Both implement `Logger`.
- **Singleton adapters log lifecycle, throw operation errors.** Pool created/closed/idle-error → `appLogger`. Query failure → `throw new AppError(...)`, no logging at the adapter.
...
- **Logger is a port with two lifetimes.** `appLogger` (singleton, on `AppContainer`) for lifecycle and out-of-request events; `logger` (request-scoped, on `event.context.scope`) for in-request code. Both implement the `Logger` port. Use cases receive `logger` via `deps` only when needed.
...
- ❌ Adding `info`/`warn`/`error` methods to the `Logger` port. evlog is wide-event based — use `set()` to accumulate context.
